...
f4a :: [Char] -> Int
f4a xs = length $ filter (\(x,y) -> (not $ isDigit x) && (x /= '.') && (isDigit y))  $ zip (' ':xs) xs
...
f4b p s f = filter(p) $ dropWhile(<s) $ takeWhile(<f) $ concatMap(\(x,y) -> [x,y]) $ iterate (\(x, y) -> (x + y, x + y + y)) (0,1)

{-- c) Дана строка текста и число n. Сформировать список, содержащий n самых часто используемых
    в строке символов. --}
f4c :: Int -> [Char] -> [Char]
f4c n = (take n) . map(\(x:xs) -> x) . reverse . sortBy(\xs ys -> compare (length xs) (length ys)) . group . sort
...
f4d :: (Num a, Ord a) => [a] -> [a]
f4d xs = map(\(_:x:_) -> x) $ filter(\(p:x:s:[]) -> (x > p) && (x > s)) $ map(take 3) $ takeWhile(\xs -> (length xs) > 2) $ tails ([head xs - 1] ++ xs ++ [last xs - 1])
